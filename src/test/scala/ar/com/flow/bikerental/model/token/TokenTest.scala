...
class TokenTest extends AnyFunSuite with Matchers {
  ...
  test("givenATransactionTokenWithExpirationDateInTheFutureWhenValidatingItThenItShouldBeValid") {
    val notExpiredToken = new Token(value = 1L, expiration = LocalDateTime.now.plusDays(1), user)
  ...
  test("givenATransactionTokenWithExpirationDateInThePastWhenValidatingItThenItShouldBeInvalid") {
    val expiredToken = new Token(value = 1L, expiration = LocalDateTime.now.minusDays(1), user)
    expiredToken.hasExpired shouldBe true
  }

  test("equals") {
    val token1 = new Token(value = 1L, expiration = LocalDateTime.now.plusDays(1), user)
    val token2 = new Token(value = 2L, expiration = LocalDateTime.now.plusDays(1), user)
