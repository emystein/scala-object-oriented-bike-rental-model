...
object TripCompletionRulesFactory {
  ...
  private def rulesByDayOfWeek: mutable.Map[DayOfWeek, Predicate[FinishedTrip]] = {
    val rules = new scala.collection.mutable.HashMap[DayOfWeek, Predicate[FinishedTrip]]
  ...
  private def allowMaxTripDuration(maxAllowedTripDuration: Duration): Predicate[FinishedTrip] = AllowedTripDurationExceededBanRule(maxAllowedTripDuration)
}